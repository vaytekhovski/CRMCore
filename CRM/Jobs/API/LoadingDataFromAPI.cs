...
namespace Jobs.API
{
    class LoadingDataFromAPI
    {
        ...
        public LoadingDataFromAPI(DateTime startDate, DateTime dateTime)
        {
        ...
        {
            isLoading = true;
            Console.WriteLine($"[{index}/{DropDownFields.Coins.Count}] Download {coin} started from {startDate} to {endDate}");
            ...
            string path = $"../../../API/json/binance-{coin}.json";
            ...
            Console.WriteLine($"{coin} download json");
            try
            {
            ...
            Console.WriteLine($"{coin} add ticker to list");
            AddTickerToLists(OrderBook, TradeHistories, TradeDeltas, ticker, coin);

            Console.WriteLine($"{coin} checking exists values");
            CheckExistValues(OrderBook, TradeHistories, TradeDeltas);

            Console.WriteLine($"{coin} add list to database");
            AddListToDataBase(OrderBook, TradeHistories, TradeDeltas);

            Console.WriteLine($"[{index}/{DropDownFields.Coins.Count}] Download {coin} ended");
            isLoading = false;
            index++;
        }
        ...
        private static void AddListToDataBase(List<OrderBookModel> orderBook, List<Business.Data.TradeHistoryModel> tradeHistories, List<TradeDeltaModel> tradeDeltas)
        {
            ...
            using (BasicContext context = new BasicContext())
            {
                ...
                Console.WriteLine("items to OrderBook added");

                context.TradeHistoryModels.AddRange(tradeHistories);
                Console.WriteLine("items to TradeHistory added");

                context.TradeDeltaModels.AddRange(tradeDeltas);
                Console.WriteLine("items to TradeDelta added");
